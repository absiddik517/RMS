    ...
    <div class="row">
      <div class="col-12">
        <Card varient="gray" body-class="p-0">
          <template #title> AcademicClasses <i v-show="loadingTable" class="fa fa-spinner fa-spin"></i>
          </template>
          ...
          <div class="row p-2 gy-2">
            ...
            <div class="col-6 d-flex justify-content-end align-items-center">
              ...
              <Dropdown animate stay header="Filter" id="filteracademicclassDropdown">
                <template #btn>
...
<th v-show="columns.short_name"> Short Name </th>
              ...
              <tbody>
                <tr v-for="(academicclass, index) in academicclasses.data">
                  <td v-show="columns.id">{{ academicclass.id }}</td>
                                      <td v-show="columns.name">{{ academicclass.name }}</td>
                    <td v-show="columns.short_name">{{ academicclass.short_name }}</td>
                  <td v-show="columns.action" class="text-right">
                    <Dropdown stay :header="academicclass.name" :id="'academicclassindex' + index">
                      <Button
                        btnDropdown
                        type="button"
                        @click="deleteacademicclass(academicclass.delete_url)"
                      >
                        <i class="fa fa-trash"></i> Delete
                      </Button>
                      <Button btnDropdown type="button" @click="showModal(academicclass)">
          ...
          <div class="p-2">
            <Pagination @traped="loadingTable = true" :items="academicclasses" />
          </div>
        </Card>
      </div>
    </div>
  </Content>
  <Modal id="academic_classes_create_modal" :title="modalTitle" varient="light">
    <template #body>
      <form @submit.prevent="submit" novalidate="novalidate">
                  <Input v-model="form.name" field="name" label="Name" :form="form" />
          <Input v-model="form.short_name" field="short_name" label="Short Name" :form="form" />
      </form>
    </template>
  ...
  <DeleteConfirm :deleteUrl="deleteUrl" item="academicclass"/>
</template>
...
export default {
  ...
  watch: {
    filter: {
      handler: _.debounce(function (state, old) {
        ...
        this.getacademicclasses(query);
      }, 1000),
      deep: true,
    },
  },
  mounted() {
    let form = document.querySelector("#academic_classes_create_modal");
  ...
  methods: {
    ...
    showModal(data = null) {
      ...
      this.modalTitle = data == null ? "Create AcademicClass" : "Update AcademicClass";
    ...
    submit() {
      this.form.clearErrors();
      var url = this.isEditing ? this.editUrl : route("classes.store");
      this.form.post(url, {
        preserveScroll: true,
        onSuccess: () => {
          this.form.reset();
